#spark-submit --driver-memory 100G --executor-cores 10 --conf spark.driver.maxResultSize=15g --conf spark.rapids.sql.enabled=true ExportAlertsAsJsonAnritsu.py



AlertsSourceParquetFile = "/WORKSPACE/Pierre/tests/Anritsu_Paris"
OutputFile = "DetectionExports/RaphaelAnritsuLvl05-29.json"

DataAcquisitionYear = 2021
...
AntennasLocInfosFile = '/rapids/notebooks/LICIT_COMMON_Folder/LICIT_INPUT/notebooks/notebooks_pierre/exports/Anritsu_Paris_LocInfos_Summarized.csv'
MinAlertLevel = 0



ExtractDateStart = "2021-05-29T06:00:00"
ExtractDateEnd = "2021-05-29T23:00:00"
...
for lti in ListTimeIntervals:
    ...
    IntervalDF  = DetectLocDF.filter( (DetectLocDF.Anomaly_Level>=MinAlertLevel) & (DetectLocDF.WeekOfYear==lti['Week']) & (DetectLocDF.MinuteWithinWeek>=lti['MinutesStart']) & (DetectLocDF.MinuteWithinWeek<lti['MinutesEnd']) )
    ...
    PandasOutput = IntervalDF.orderBy(f.col('MinuteWithinWeek').asc()).drop('WeekOfYear').toPandas()
